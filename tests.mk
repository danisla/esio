...
test-%-day-offline: $(TEST_DEPS)
	...
	@if [ "$(RES)" != "$(EXP_RES)" ]; then echo "ERROR: Expected status code '$(EXP_RES)' but saw: '$(RES)'" ; exit 1; fi
...
test-%-day-delete: $(TEST_DEPS)
	$(eval EXP_RES := 202)
	$(eval RES := $(shell curl --silent --output /dev/stderr --write-out "%{http_code}" -XDELETE "http://$(APP_HOST):$(APP_PORT)/$(DOY_START_TS)/$(shell echo $(DOY_START_TS) + 3600*24*$* | bc)?repo_pattern=$(TEST_REPO_PATTERN_DAILY)&resolution=day"))
	@if [ "$(RES)" != "$(EXP_RES)" ]; then echo "ERROR: Expected status code '$(EXP_RES)' but saw: '$(RES)'" ; exit 1; fi
